...
def save_solution(rake_paths: Dict[Tuple[int, int], int], fname: str) -> None:
    ...
    with open(fname_path, "wb", encoding="utf-8") as file:
...
def create_terrain(fname: str, max_runs: int, show: bool = False) -> str:
    ...
    Args:
        fname (str): name of the file that is going to be created
        max_runs (int): max number of attempts to find solution with evo. alg.
        show (bool, optional): Print created terrain into console.
            Defaults to False.
    ...
    # mark remaining unraked space with -2
    terrained_map = [
        [str(i) if i else "-2" for i in subarray] for subarray in map_list
    ]

    save_solution(rake_paths, fname)  # after evolutionize checkout -> check docstring here
...
def create_maps(
    ...
    previous one was run. All remaining stages will run after that.
